...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "subscribe3get" {
		// the old "Query" is now implemented in invoke
		return t.subscribe3get(stub,args)
	} else if function == "mysubscribe3get" {
		// the old "Query" is now implemented in invoke
		return t.mysubscribe3get(stub,args)
	}
...
func (t *SimpleChaincode) subscribe3(stub shim.ChaincodeStubInterface, args []string, name string) pb.Response {
	...
	txid := stub.GetTxID()
	compositeIndexName := "security~org~txID"
	compositeIndexName2 := "org~security~txID"

	// Create the composite key that will allow us to query for all deltas on a particular variable
	compositeKey, compositeErr := stub.CreateCompositeKey(compositeIndexName, []string{aSub,a, txid})
	if compositeErr != nil {
		return shim.Error(fmt.Sprintf("Could not create a composite key for %s: %s", name, compositeErr.Error()))
	}
	
	compositeKey2, compositeErr2 := stub.CreateCompositeKey(compositeIndexName2, []string{a,aSub, txid})
	...
	return shim.Success([]byte(fmt.Sprintf("Successfully added %s%s to %s", a,aSub,txid)))
}
//--------------
func (s *SimpleChaincode) subscribe3get(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	deltaResultsIterator, deltaErr := APIstub.GetStateByPartialCompositeKey("security~org~txID", []string{name})
	if deltaErr != nil {
...
func (s *SimpleChaincode) mysubscribe3get(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	// Check we have a valid number of args
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments, expecting 1")
	}
